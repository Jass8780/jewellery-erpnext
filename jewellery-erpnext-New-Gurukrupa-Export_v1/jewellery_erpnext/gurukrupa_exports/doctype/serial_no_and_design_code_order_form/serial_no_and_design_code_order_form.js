...
frappe.ui.form.on("Serial No and Design Code Order Form", {
	...
	setup: function (frm) {
		frm.set_query("diamond_quality", function (doc) {
			if (!doc.customer_code) {
				frappe.throw(__("Please select customer code."));
			}
			return {
...
frappe.ui.form.on("Serial No and Design Code Order Form Detail", {
	...
	item: function (frm, cdt, cdn) {
		...
		if (d.item) {
			frappe.db.get_value(
				"Item",
				{ name: d.item },
				["item_category", "item_subcategory", "setting_type"],
				function (value) {
...
function show_hide_field(frm, cdt, cdn, field, hidden) {
	...
	var df = frappe.utils.filter_dict(
		frm.fields_dict["order_details"].grid.grid_rows_by_docname[cdn].docfields,
...
function set_order_type_from_design_by(frm) {
	if (frm.doc.design_by == "Customer Design") {
		frm.doc.order_type = "Customer Order";
	} else {
		frm.doc.order_type = "Stock Order";
	}
