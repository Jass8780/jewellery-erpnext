erpnext.SerialNoBatchSelector = class SerialNoBatchSelector {
	...
	setup() {
		...
		this.inventory_type = this.item.inventory_type;
	...
	batch_exists(batch) {
		...
		return batches && batches.includes(batch) ? true : false;
	}
	...
	get_batch_fields() {
		var me = this;
		return [
			{ fieldtype: "Section Break", label: __("Inventory Dimension") },
			{
				fieldname: "inventory_type",
				fieldtype: "Link",
				options: "Inventory Type",
				label: __("Inventory Type"),
				default: me.inventory_type,
			},
			{ fieldtype: "Column Break" },
			{
				fieldname: "customer",
