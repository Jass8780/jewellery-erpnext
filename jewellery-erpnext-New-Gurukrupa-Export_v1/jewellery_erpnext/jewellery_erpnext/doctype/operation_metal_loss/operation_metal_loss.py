...
class OperationMetalLoss(Document):
	...
	def update_gross_weights(self):
		...
		data = frappe.db.get_list(
			"Job Card", filters=filters, fields=["loss_gold_weight", "loss_finding_weight", "metal_purity"]
		)
		self.get_loss_by_purity(data)
		self.total_metal_loss = sum(
			i.get("loss_gold_weight") + j.get("loss_finding_weight") for i, j in zip(data, data)
		)
