...
class MeltingLot(Document):
	...
	def validate(self):
		if self.order_details:
			for item in self.order_details:
				name, qty, production_qty, balance_production_qtys = frappe.db.get_value(
					"Design Order Detail",
					{"parent": item.reference_name, "parenttype": item.reference_type, "item": item.item},
					["name", "qty", "production_qty", "balance_production_qty"],
				)
				balance_qty = flt(qty - production_qty)
				item.remaining_qty = (
					flt(balance_qty - item.production_qty) if flt(balance_qty - item.production_qty) >= 0 else 0
				)
					...
					if balance_production_qtys == balance_production_qty:
						frappe.db.set_value(
							...
							name,
							{
								...
								"production_qty": flt(production_qty) + flt(item.production_qty),
								"balance_ready_qty": qty,
							},
						)
		...
		if self.order_bunch_details:
			for item in self.order_bunch_details:
				(
					...
					production_length,
					balance_pro_len,
					balance_production_wei,
				) = frappe.db.get_value(
					"Design Order Bunch Detail",
					{
						"parent": item.reference_name,
						"parenttype": item.reference_type,
					...
					[
						...
						"production_weight",
						"production_length",
						"balance_production_length",
						"balance_production_weight",
					],
				)
				balance_length = flt(length - production_length)
				balance_weight = flt(weight - production_weight)
				item.remaining_weight = (
					flt(balance_weight - item.production_weight)
					if flt(balance_weight - item.production_weight) >= 0
					else 0
				)
				item.remaining_length = (
					flt(balance_length - item.production_length)
					if flt(balance_length - item.production_length) >= 0
					else 0
				)
				...
				if weight != production_weight:
					if balance_production_wei == balance_production_weight:
						frappe.db.set_value(
							...
							get_id,
							{
								"balance_production_weight": balance_production_weight,
								"production_weight": flt(production_weight) + flt(item.production_weight),
								"balance_ready_weight": weight,
							},
						)
				...
				if length != production_length:
					if balance_pro_len == balance_production_length:
						frappe.db.set_value(
							...
							get_id,
							{
								"balance_production_length": balance_production_length,
								"production_length": flt(production_length) + flt(item.production_length),
								"balance_ready_length": length,
							},
						)
	...
	def on_submit(self):
		from jewellery_erpnext.jewellery_erpnext.doctype.operation_card.operation_card import (
			make_operation_card,
		)
...
def make_melting_lot(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Design Order": {
				"doctype": "Melting Lot",
				"field_map": {"purity": "product_purity"},
			},
			"Design Order Detail": {
				"doctype": "Melting Lot Design Order Detail",
				"field_map": {"parent": "reference_name", "parenttype": "reference_type"},
			...
			"Design Order Bunch Detail": {
				...
				"condition": lambda doc: doc.production_length < doc.length
				or doc.production_weight < doc.weight,
			},
		},
		target_doc,
	)
