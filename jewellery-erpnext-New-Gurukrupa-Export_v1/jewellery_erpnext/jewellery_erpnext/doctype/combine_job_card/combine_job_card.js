...
frappe.ui.form.on("Combine Job Card", {
	...
	refresh(frm) {
		if (!frm.doc.__islocal && !frm.doc.details.find((jc) => jc.submitted)) {
			...
			let se_list = frm.doc.details
				.filter((jc) => jc.stock_entry && !jc.submitted)
				.map((ob) => [ob.stock_entry, ob.name]);
			if (se_list.length > 0) {
				frm.add_custom_button(__("Submit Material Transfer"), () => {
					frm.call({
						...
						callback: function (r) {
							...
							if (res.length > 0) {
								frappe.show_alert({
									message: __("Something went wrong. Check Error Log"),
									indicator: "orange",
								});
							} else
								frappe.show_alert({
									message: __("Documents submitted successfully"),
									indicator: "green",
								});
						},
					});
	...
	make_stock_entry: function (frm) {
		let jc_list = frm.doc.details
			.filter((jc) => !jc.stock_entry)
			.map((ob) => [ob.job_card, ob.name]);
		frappe.call({
					...
					frappe.msgprint(
						__(
							"Stock Entries Created: " +
								res
									.map((ob) =>
										frappe.utils.get_form_link("Stock Entry", ob, true)
									)
									.join(", ")
						)
					);
				else frappe.msgprint(__("No entries created"));
			},
		});
	},
			...
			fields: [
				{
					...
					onchange: function () {
						frappe.db.get_value(
							...
							"combine_job_card",
							function (r) {
								dialog.fields_dict.combine_job_card.set_value(
									r.combine_job_card || 0
								);
				...
				{
					fieldtype: "Check",
					label: __("Is Combined operation"),
					fieldname: "combine_job_card",
					read_only: 1,
				},
				{
					fieldtype: "Link",
					label: __("To Combine Job Card"),
					fieldname: "to_combine_job_card",
					reqd: 0,
					options: "Combine Job Card",
					get_query: () => {
						return {
							filters: {
								operation: dialog.get_value("next_operation"),
				...
				{
					fieldtype: "Link",
					label: __("To Job Card"),
					fieldname: "to_job_card",
					reqd: 0,
					options: "Job Card",
					hidden: 1,
					get_query: () => {
