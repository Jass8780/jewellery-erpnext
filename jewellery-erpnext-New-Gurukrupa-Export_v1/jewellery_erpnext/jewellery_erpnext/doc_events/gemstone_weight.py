...
from jewellery_erpnext.jewellery_erpnext.doc_events.diamond_weight import (
...
def update_item_uom_conversion_from_gemstone():
	...
	for row in data:
		...
		attribute_filters = {
			"Gemstone Type": gemstone_weight_doc.gemstone_type,
			"Stone Shape": gemstone_weight_doc.stone_shape,
			"Gemstone Grade": gemstone_weight_doc.gemstone_grade,
			"Gemstone Size": gemstone_weight_doc.gemstone_size,
		}
		...
		if item_list:
			for item in item_list:
				...
				for d in to_remove:
					doc.remove(d)
				doc.append("uoms", {"uom": "Pcs", "conversion_factor": gemstone_weight_doc.weight})
