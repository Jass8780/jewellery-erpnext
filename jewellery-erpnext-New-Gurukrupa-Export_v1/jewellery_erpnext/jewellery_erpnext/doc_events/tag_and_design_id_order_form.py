...
def create_tag_and_design_order(self):
	for row in self.order_details:
		...
		doc.save()
		frappe.msgprint(
			_("Tag and Design ID Order {0} created").format(
				get_link_to_form("Tag and Design ID Order", doc.name)
			)
		)
...
def delete_auto_created_tag_and_design_order(self):
	for row in frappe.get_all(
		"Tag and Design ID Order", filters={"tag_and_design_id_order_form": self.name}
	):
