...
def validate(self, method):
	if self.is_new():
		self.wip_warehouse = frappe.db.get_value(
			"Operation Warehouse", {"parent": self.operation, "company": self.company}, "warehouse"
		)
	render_job_card_data(self)
	add_qc_params(self)
	...
	try:
		self.workstation = (
			frappe.db.get_value("Operation", self.operation, "workstation") or self.operation
		)
		data = frappe.db.get_value(
			"Workstation", self.workstation, ["workstation_type", "employee"], as_dict=True
		)
...
def set_waxing_weight(self):
	try:
		if self.operation == "Waxing 1":
			frappe.db.set_value(
				"Job Card",
				{"work_order": self.work_order, "operation": "Tree Making"},
				"wax_loss",
				self.wax_weight,
			)
			frappe.db.set_value(
				"Job Card",
				{"work_order": self.work_order, "operation": "Casting"},
				"wax_loss",
				self.wax_weight,
			)
	except Exception as e:
...
def onsubmit(self, method):
	if self.balance_gross != 0:
		return frappe.throw(
			_("Difference Between Total In Gross Weight And Total Out Gross Weight Should Be 0")
		)
	if self.operation == "Tagging":
		if self.get("tag"):
			...
			frappe.throw(_("Tag No. is missing"))
...
def add_qc_params(self):
	...
	conditions = (QCParameter.category.isnull() | (QCParameter.category == categories)) & (
		QCParameter.operation.isnull() | (QCParameter.operation == self.operation)
	)
	query = frappe.qb.from_(QCParameter).select(QCParameter.parameter).where(conditions)
...
def set_warehouse(operation, company):
	if operation:
		warehouse = frappe.get_all(
			"Operation Warehouse", {"parent": operation, "company": company}, "warehouse"
		)
		if warehouse:
			# if len(warehouse) == 1:
...
def create_new_job_card(
	company, posting_date, for_quantity, work_order, bom_no, operation, workstation, employee
):
	...
	doc.operation_row_number = frappe.db.get_value(
		"Work Order Operation", {"parent": work_order_doc.name, "operation": operation}, "name"
	)
...
@frappe.whitelist()
def create_stock_entry_material_receipt(
	company,
	work_order,
	to_warehouse,
	from_job_card,
...
def create_internal_transfer(
	...
	from_job_card=None,
	to_job_card=None,
	to_combine_job_card=None,
	from_combine_job_card=None,
	next_operation=None,
	balance_gross=None,
):
	...
	frappe.msgprint(
		_("Job Card Out Weight {0} created").format(
			get_link_to_form("Job Card Internal Transfer", doc.name)
		)
	)


def validate_out_weight(from_job_card, to_combine_job_card, gross_wt, balance_gross):
	if not from_job_card and not to_combine_job_card:
		return
	frappe.msgprint(str(balance_gross))


def _delete_remaining_job_cards(work_order, from_job_card):
	jc_list = frappe.db.get_list(
		"Job Card",
		{"work_order": work_order, "docstatus": 0, "name": ("!=", from_job_card)},
		["name", "operation"],
	)
	for jc in jc_list:
		frappe.db.delete("Job Card", jc.get("name"))
		frappe.db.delete(
			"Work Order Operation", {"parent": work_order, "operation": jc.get("operation")}
		)
	return
...
def set_reference_child_table_name(self):
	for item in self.items:
		if item.reference_doctype and item.reference_docname:
			return
		doctype = frappe.db.get_value(
			"Work Order Item", {"parent": self.work_order, "item_code": item.item_code}, "reference_doctype"
		)
		docname = frappe.db.get_value(
			"Work Order Item", {"parent": self.work_order, "item_code": item.item_code}, "reference_docname"
		)
		pcs = frappe.db.get_value(
			"Work Order Item", {"parent": self.work_order, "item_code": item.item_code}, "pcs"
		)
...
def _get_in_weights_from_combined_job_card(self):
	...
	"""
	if not self.in_combined_job_card:
		return
...
def _get_in_weight_from_prev_combined_jc_self_in_cj(self):
	...
	query = (
		frappe.qb.from_(JobCardInternalTransferItem)
		.left_join(CombineJobCard)
		.on(CombineJobCard.name == JobCardInternalTransferItem.from_combine_job_card)
		.left_join(CombineJobCardDetail)
		.on(CombineJobCardDetail.parent == CombineJobCard.name)
		.left_join(JobCard)
		.on(JobCard.name == CombineJobCardDetail.job_card)
		.select(
			JobCard.name,
			JobCardInternalTransferItem.item_code,
	...
	for i in internal_in_weight_data:
		...
		for jc in cj_doc.details:
			if frappe.db.get_value("Job Card", jc.get("job_card"), "work_order") == self.work_order:
				...
				net_wt = (
					frappe.db.get_value("Job Card", jc.get("job_card"), "out_gold_weight")
					+ finding_weight
					- balance_gross
				)
				data_dict = {
...
def _render_out_weights(self, out_weights_data):
	...
	out_fine_weight = sum(
		(item.get("net_wt") * item.get("purity") / 100) for item in out_weights_data
	)
		...
		if internal_in_weight_data:
			for scrap_item in cj_doc.scrap_items:
				scrap_items = self.get(
					"scrap_items", {"item_code": scrap_item.get("item_code"), "parent": self.name}
				)
...
def get_wastages_from_prev_combined_jc(self):
	...
	query = (
		frappe.qb.from_(JobCardInternalTransferItem)
		.left_join(CombineJobCardDetail)
		.on(CombineJobCardDetail.parent == JobCardInternalTransferItem.from_combine_job_card)
		.left_join(JobCard)
		.on(JobCard.name == CombineJobCardDetail.job_card)
		.select(
			JobCardInternalTransferItem.item_code,
	...
	if internal_in_weight_data and internal_in_weight_data[0].get("from_combine_job_card"):
		...
		for scrap_item in cj_doc.scrap_items:
			...
			quantity = (
				scrap_item.get("stock_qty")
				/ cj_doc.total_in_gross_weight
				* internal_in_weight_data[0].get("gross_wt")
			)
			if not scrap_items:
				self.append(
					"scrap_items",
...
def get_totals(self):
	...
	total_wastage = (
		self.loss_gold_weight
		+ self.loss_diamond_weight / 5
		+ self.loss_gemstone_weight / 5
		+ self.loss_finding_weight
	)
	fine_wastage = (self.loss_gold_weight + self.loss_finding_weight) * (
		float(self.metal_purity or 0) / 100
	)
	...
	html_content = frappe.render_template(
		total_html,
		{
			"total_in_gross_weight": round(self.total_in_gross_weight, 3),
...
def set_total_in_weight_fields(self, internal_in_gross_wt):
	internal_in_gross_wt = (
		self.in_gold_weight
		+ self.in_diamond_weight / 5
		+ self.in_gemstone_weight / 5
		+ self.in_finding_weight
		+ self.in_other_weight
	)
...
def set_total_out_weight_fields(self):
	...
	if self.in_combined_job_card:
		self.out_gross_weight = (
			self.out_gold_weight
			+ (self.in_diamond_weight / 5 - self.loss_diamond_weight / 5)
			+ (self.in_gemstone_weight / 5 - self.loss_gemstone_weight / 5)
			+ self.out_finding_weight
			+ self.out_other_weight
		)
	self.total_out_gross_weight = self.out_gross_weight + wastage_loss
	self.total_out_fine_weight = (
		self.out_fine_weight + (self.loss_gold_weight + self.loss_finding_weight)
	) * (float(self.metal_purity or 0) / 100)
...
def stock_entry_detail(item):
	...
	query = (
		...
		.on(
			(JobCardScrapItem.parent == JobCard.name)
			& (JobCardScrapItem.item_code == StockEntryDetail.item_code)
		)
		.select(
			JobCard.name,
			StockEntryDetail.item_code,
...
def make_stock_return(source_name, target_doc=None):
	...
	def update_item(source, target, source_parent):
		target.to_warehouse = frappe.db.get_value(
			"Work Order", source_parent.work_order, "wip_warehouse"
		)
